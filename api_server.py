...
from utils.book_converter import BookConverter # <--- ИМПОРТИРОВАН КОНВЕРТЕР
...
async def startup_event():
    ...
    try:
        print("Инициализация AI-пайплайнов при старте сервера...")
...
# Модели для данных о проектах и файлах
...
def _run_task_wrapper(task_id: str, target_func, **kwargs):
    try:
        ...
        progress_callback = lambda p, m: _update_task_progress(task_id, p, m)
...
def _start_task(target_func, background_tasks_runner: BackgroundTasks, **kwargs):
    ...
    background_tasks[task_id] = {"status": "queued", "progress": 0.0, "message": "Задача поставлена в очередь."}
...
async def import_project(file: UploadFile = File(...)):
    """
    NEW: Загружает файл книги (.txt, .epub) и преобразует его в структуру проекта.
    """
    ...
    try:
        ...
        # 2. Используем BookConverter для обработки файла
        ...
        # В случае ошибки при конвертации, удаляем созданную папку проекта, если она есть
        project_name = temp_file_path.stem
        ...
        # 3. Удаляем временный файл после обработки
...
async def get_book_artifact(book_name: str, artifact_name: BookArtifactName):
    ...
    artifact_path = getattr(context, f"{artifact_name.name}_file", None)
...
if __name__ == "__main__":
    print("="*50)
    print("🚀  ДЛЯ ЗАПУСКА СЕРВЕРА ВЫПОЛНИТЕ В ТЕРМИНАЛЕ:")
