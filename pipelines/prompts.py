...
5.  **ДИНАМИКА ПОВЕСТВОВАНИЯ (ДРОБЛЕНИЕ ТЕКСТА - ЖЕСТКОЕ ПРАВИЛО):**
    -   **АБСОЛЮТНЫЙ ЛИМИТ:** Один блок `narration` **НЕ ДОЛЖЕН превышать 2-3 предложения (около 30-40 слов)**. 
    -   Если в оригинальном тексте идет большой абзац на 4-7 предложений, ты **ОБЯЗАН** разбить его на несколько отдельных, подряд идущих объектов `narration` в JSON.
    -   Невыполнение этого правила приведет к поломке игры из-за слишком длинных экранов с текстом!
...
- `ambient`: ID фонового звука. Оценивай КАЖДУЮ реплику. Если атмосфера изменилась (началась ссора на повышенных тонах, или герой вышел на улицу) — ОБЯЗАТЕЛЬНО выбери новый звук из меню. Если тишина — "none".
