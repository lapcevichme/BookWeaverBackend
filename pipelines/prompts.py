...
def format_character_patch_prompt(
    ...
    ОБЪЕДИНЕНАЯ ЛОГИКА: Hierarchy Protection + Detailed Timeline Instructions.
    """
...
=== 1. ПРАВИЛА ВЫБОРА ИМЕНИ (Naming Protocol) ===
...
- Обязательно объясни выбор в `naming_reasoning`.

=== 2. ТАЙМЛАЙНЫ (State Timelines) - КЛЮЧЕВОЙ МОМЕНТ ===
Заполняй эти поля ТОЛЬКО если есть РЕАЛЬНЫЕ изменения. Иначе оставь `null`.

- **timeline_voice_update (ГОЛОС/ВОЗРАСТ):**
  - Заполняй, если прошел "таймскип" (годы) или голос изменился (магия, болезнь).
  - `search_tags`: Напиши 5-7 тегов на АНГЛИЙСКОМ для поиска голоса (например: "old man, raspy, slow, wise, russian").
...
- **role_tier:** Если персонаж типа "Деревенщина" — ставь `background`. Если имеет имя — `minor` или выше.
- **gender:** ОБЯЗАТЕЛЬНО укажи пол (male, female, other) для новых персонажей, если это понятно из контекста (местоимения "он/она", окончания глаголов).
